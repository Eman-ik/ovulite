...
@keyframes bubbleFloat {
  ...
  @keyframes etherealFocus {
    0%   { opacity: 0; filter: blur(30px); letter-spacing: 0.4em; transform: scale(1.05); }
    100% { opacity: 1; filter: blur(0);    letter-spacing: -0.03em; transform: scale(1); }
  }
  ...
  .welcome-reveal {
    animation: etherealFocus 2.5s cubic-bezier(0.19, 1, 0.22, 1) forwards;
  }
  .subtitle-reveal {
    opacity: 0;
    animation: subtitleFade 1.5s cubic-bezier(0.19, 1, 0.22, 1) 0.8s forwards;
  }
`;
...
export function LoginPage() {
      ...
      const errorMessage =
        err instanceof Error
          ...
          : typeof err === "object" && err !== null && "response" in err
            ? (err as { response?: { data?: { detail?: string } } }).response?.data?.detail || "Login failed. Please check your credentials."
  ...
  const inputStyle = {
    background: "#ffffff",
    border: "1px solid rgba(31, 77, 68, 0.3)",
  ...
  style={{
    background: "linear-gradient(135deg, #1F4D44 0%, #2a6b60 40%, #1a3a33 70%, #0f2420 100%)",
  }}
>
          {!isDark && (
            <>
          ...
          {/* Welcome Back — centered in the green panel */}
          <div
            className="welcome-reveal pointer-events-none absolute inset-0 flex items-center justify-center"
            style={{ zIndex: 10 }}
          >
            <div style={{ textAlign: 'center', padding: '0 48px' }}>
              <h1 style={{
                ...
                lineHeight: 1.05,
                textShadow: '0 4px 32px rgba(0,0,0,0.4), 0 1px 0 rgba(255,255,255,0.15)',
                margin: 0,
              }}>
                Welcome Back
              </h1>
        ...
        <div className="flex w-full md:w-1/2 items-center justify-center px-6 py-8" style={{
          background: isDark
            ? "#060608"
            : "linear-gradient(to bottom, #F7F3ED 0%, #D9CDBF 60%, #F7F3ED 100%)",
          transition: "background 0.5s ease",
        }}>
          <div
            className="w-full max-w-[680px] rounded-[2rem] p-8 md:p-12 backdrop-blur-3xl transition-all duration-500"
            style={{
              ...
              boxShadow: isDark
                ...
                : "0 8px 48px rgba(255, 213, 128, 0.2)",
            }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <label className="block">
              <span className="mb-2 ml-1 block text-xs font-bold uppercase tracking-wider" style={{ color: isDark ? "#d4d4d8" : "#1e293b", fontSize: "13px", fontFamily: '"Inter", sans-serif', fontWeight: 500 }}>
            ...
            <div className="flex items-center justify-between px-1">
              <label className="flex cursor-pointer items-center gap-2">
                <input type="checkbox" className="h-4 w-4 rounded" style={{ accentColor: isDark ? "#f97316" : "#3b82f6" }} />
                <span className="text-sm" style={{ color: isDark ? "#71717a" : "#64748b", fontSize: "14px", fontFamily: '"Inter", sans-serif', fontWeight: 400 }}>
                  Remember me
                </span>
              </label>
              <button type="button" className="text-sm font-semibold" style={{ color: isDark ? "#fb923c" : "#000000", fontSize: "14px", fontFamily: '"Inter", sans-serif', fontWeight: 600 }}>
                Forgot password?
              </button>
            </div>
          ...
          <p className="mt-10 text-center text-sm" style={{ color: isDark ? "#52525b" : "#64748b", fontSize: "14px", fontFamily: '"Inter", sans-serif', fontWeight: 400 }}>
            Don't have an account?{" "}
            <button type="button" onClick={() => navigate("/signup")} className="font-bold underline underline-offset-4" style={{
